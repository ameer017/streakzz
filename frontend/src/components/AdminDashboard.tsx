...
import { Users, Trophy, Calendar, Search, Mail } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'projectCount' | 'streakCount' | 'joinedAt'>('streakCount');
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-gray-600">Manage participants and monitor platform activity</p>
        </motion.div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  >
                    Projects {sortBy === 'projectCount' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
                  <th
                    className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                    onClick={() => handleSort('streakCount')}
                  >
                    Streak {sortBy === 'streakCount' && (sortOrder === 'asc' ? '↑' : '↓')}
                  </th>
                  <th
                    className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredAndSortedParticipants.map((participant, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStreakColor(participant.streakCount)}`}>
                        {participant.streakCount} days ({getStreakLabel(participant.streakCount)})
                      </span>
                    </td>
